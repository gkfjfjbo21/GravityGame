@tool
extends Node2D
## Gravity Game — прототип уровня.
## Колёсико мыши — сила гравитации, A/D — ходьба, Space — прыжок, Shift — dash, R — рестарт.
...
const C_FLIP   := Color(0.20, 0.80, 0.90, 0.85)  # сфера флипа гравитации (авто)
const C_IFLIP  := Color(0.90, 0.50, 0.20, 0.85)  # сфера флипа (интерактивная, пробел)
const C_STICKY := Color(0.85, 0.65, 0.10)         # липкие поверхности (мёд)
const C_NOGRV  := Color(0.55, 0.20, 0.70, 0.25)   # антигравитационные зоны
const C_CONV   := Color(0.30, 0.75, 0.55)           # конвейеры
...
func _spring_hit(body: Node2D) -> void:
	if Engine.is_editor_hint():
		return
	if body == player:
		# Сила пружины зависит от гравитации: слабая гравитация = сильнее отскок
...
# ── Конвейеры (зелёно-бирюзовые, двигают игрока по поверхности) ──
func _build_conveyors() -> void:
	# speed > 0 = вправо, < 0 = влево
...
# ── Липкие поверхности (оранжевые, замедляют + слабый прыжок) ──
...
func _sticky_rect(pos: Vector2, sz: Vector2) -> void:
	...
	# Area2D чуть больше для детекции входа/выхода
	var area := Area2D.new()
	area.position = pos
	var shape := RectangleShape2D.new()
...
# ── Антигравитационные зоны (блокируют колесо мыши) ──
...
func _antigrav_zone(pos: Vector2, sz: Vector2) -> void:
	...
	col.shape = shape
	area.add_child(col)
	# Визуал — полупрозрачный фиолетовый прямоугольник
	var vis := ColorRect.new()
	vis.size = sz
	vis.position = -sz / 2
	vis.color = C_NOGRV
	...
	# Внутренность поверх рамки (чуть меньше)
...
# ── Сферы флипа гравитации (многоразовые, бирюзовые) ──
...
func _gravity_flip_sphere(pos: Vector2) -> void:
	...
	if vis.get_script() == null:
		# Фоллбэк — простой круг через _draw
...
# ── Интерактивные сферы флипа (оранжевые, нажать пробел) ──
...
# ── Вспомогательный класс для рисования круга ────────
...
func _update_hud(multiplier: float, direction: int) -> void:
	...
	# Шкала: 0.2x внизу, 5.0x вверху (снизу заполняется)
